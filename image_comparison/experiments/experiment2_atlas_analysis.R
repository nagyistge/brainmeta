...
input_files =list.files("/home/vanessa/Documents/Work/BRAINMETA/IMAGE_COMPARISON/analysis/atlas_scores_all",pattern="pearson*")
datadir = "/home/vanessa/Documents/Work/BRAINMETA/IMAGE_COMPARISON/analysis/atlas_scores_all"
setwd(datadir)
...
colnames(gs_flat)[c(1,4,5)] = c("X","strategy","n_roi")
sizes_flat = melt(sizes,id.vars=c("X","strategy","n_roi"))
# Remove values of NA - when comparing an image to itself
sizes_flat = sizes_flat[-which(is.na(sizes_flat$value)),]
pearsons_flat = melt(pearsons,id.vars=c("X","strategy","n_roi"))
# Remove values of NA - when comparing an image to itself
...
savedir = "/home/vanessa/Documents/Work/BRAINMETA/IMAGE_COMPARISON/img/roi"
plot.new()
for (n in nroi){
  ...
  plot1 = ggplot(both,aes(x=value, fill=strategy)) + 
    ...
    ylim(0,13) +
    ylab("Density") + 
    xlab(paste(n,"regions"))
  # We also need the size data in the next plot
  size_subset = sizes_flat[which(sizes_flat$n_roi==n),]
  plot2 = ggplot(size_subset,aes(y=value,x=strategy, fill=strategy)) + 
    geom_boxplot(alpha=0.25) + 
    ylab("number voxels") + 
    ylim(0,max(size_subset$value)) +
    xlab("total size of regions") +
    guides(fill=FALSE)
  g = arrangeGrob(plot2, plot1, ncol=2)
  ggsave(file=paste(savedir,"/density_plot/pearson_roi_denssize_",n,".png",sep=""),g)
}
...
for (image_id in image_ids){
  ...
  sorted_gs = sort(abs(gs[which(gs$ID==image_id),-c(1)]),decreasing=TRUE)
  ...
  for (n in unique(testadjust$n_roi)){
    ...
    pdr = pdr[which(pdr$n_roi==n),-which(colnames(pdr)=="n_roi")]
    pdr = gsub("X","",names(sort(abs(pdr),decreasing=TRUE)))
    pdr = gsr[pdr]      
    s = get_sets(gsr,pdr)
    ...
    pir = pir[which(pir$n_roi==n),-which(colnames(pir)=="n_roi")]
    pir = gsub("X","",names(sort(abs(pir),decreasing=TRUE)))
    pir = gsr[pir]      
    s = get_sets(gsr,pir)
    tau = cor.test(s$set1,s$set2, method = c("kendall"), conf.level = 0.95)
    ...
    bmr = bmr[which(bmr$n_roi==n),-which(colnames(bmr)=="n_roi")]
    bmr = gsub("X","",names(sort(abs(bmr),decreasing=TRUE)))
    bmr = gsr[bmr]      
    s = get_sets(gsr,bmr)
    tau = cor.test(s$set1,s$set2, method = c("kendall"), conf.level = 0.95)
...
colnames(df) = c("imageid","strategy","tau_pvalue","tau","n_roi")
rownames(df) = seq(1,nrow(df))
tmp = as.data.frame(df,stringsAsFactors=FALSE)
tmp$tau_pvalue = as.numeric(tmp$tau_pvalue)
tmp$n_roi = as.numeric(tmp$n_roi)
save(tmp,file=paste(datadir,"/single_tau_regional_scores_uncorrected.Rda",sep=""))

# This function returns FDR corrected qvalues (c) for each threshold
counts = c()
for (n in sort(nroi)){
    subset = tmp[which(tmp$n_roi==n),]
    c = plot_pval_region(subset,n_roi,savedir,0.05)
