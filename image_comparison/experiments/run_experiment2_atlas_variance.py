...
atlas_file = "/scratch/users/vsochat/DATA/ATLAS/AAL/ROI_MNI_V4.nii"
atlas_labels = "/scratch/users/vsochat/DATA/ATLAS/AAL/ROI_MNI_V4.txt"
...
atlas_labels = pandas.read_csv(atlas_labels,sep="\t",header=None)
...
for num_regions in range(0,number_regions):
  ...
  if not os.path.exists(outfile):
    ...
    filey.writelines("python /home/vsochat/SCRIPT/python/brainmeta/image_comparison/experiments/experiment2_atlas_variance.py %s %s %s %s %s %s %s" %(atlas_file,num_regions,atlas_labels,input_file,outfile,standard,gs_file))
    filey.close()
